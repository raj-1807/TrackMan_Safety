...
const ZoneTable: React.FC<ZoneTableProps> = ({ columns, data, onView, onEdit, onDelete }) => {
  return (
    <div className="overflow-x-auto">
            ...
            <tr key={i} className="hover:bg-blue-50/30 transition-colors group">
              ...
              <td className="px-5 py-3.5 text-right">
                <div className="flex items-center justify-end gap-1">
                  ...
                  {onDelete && (
                    <button onClick={() => onDelete(row)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all cursor-pointer">
      ...
      {data.length === 0 && (
        <div className="py-12 text-center text-sm text-slate-400">No zones found</div>
      )}
    </div>
  );
};
